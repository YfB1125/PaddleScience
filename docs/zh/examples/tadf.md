...
!!! note
    ...
    3. 开始训练、评估前，请安装 `rdkit` 等，相关依赖请执行`pip install requirements.txt`安装。
...
每项分子性质数据包括性质数据文件（例如angle.dat）和SMILES数据文件（smis.txt）。首先依据配置文件的信息逐行对性质数据文件和SMILES数据文件进行加载，随后通过分子指纹生成、异常处理、降维处理将SMILES分子式转换为可用于机器学习的特征向量，完成数据集的准备，用 PaddleScience 代码表示如下
